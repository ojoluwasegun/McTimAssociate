...
function tryAssignAdvisor(io, chatId) {
  ...
  const sla = isWeekend()
    ...
    : "within 2 hours";
  const fallback = `It looks like none of our advisors are online right now. For a faster response, please call our office, or send us an email - we reply ${sla}. Would you like me to leave your details for an advisor to follow up?`;
...
router.post("/chats/:id/resources", requireAuth("advisor"), (req, res) => {
  ...
  const label = kind === "receipt" ? "receipt" : kind === "calendar" ? "calendar invite" : kind;
