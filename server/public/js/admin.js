(function () {
  ...
  async function loadBranding() {
    try {
      ...
      companyNameLabel.textContent = b.company_name || "McTimothy Associates";
      if (brandBadge) {
  ...
  function connectSocket() {
    socket = io();
    socket.emit("admin_hello", { token });
    socket.on("advisor_status_changed", ({ advisorId, status }) => {
      const pill = document.querySelector(`[data-advisor-status="${advisorId}"]`);
      if (pill) {
        pill.textContent = status;
        pill.className = `pill ${status === "active" ? "green" : status === "busy" ? "yellow" : "grey"}`;
      }
    });
  }
  ...
  function showView(view) {
    ...
    const renderers = { analytics: renderAnalytics, advisors: renderAdvisors, visitors: renderVisitors, logs: renderLogs, faqs: renderFaqs, brochures: renderBrochures, quick: renderQuick, replies: renderReplies, questions: renderUnansweredQuestions, calendar: renderCalendar, templates: renderTemplates, proposals: renderProposals, profile: renderProfile };
    (renderers[view] || (() => {}))();
  }
  ...
  async function renderAnalytics() {
    ...
    content.innerHTML = `
      ...
      <div class="grid cols-2" style="margin-top:16px;">
        <div class="card">
          ...
          <div style="height:240px;">${d.perAdvisor.length ? `<canvas id="advisorPieChart"></canvas>` : `<div class="empty">No advisors yet</div>`}</div>
        </div>
        <div class="card">
          <h4>Cases attended by day of week</h4>
          <div style="height:240px;"><canvas id="weekdayBarChart"></canvas></div>
        </div>
      </div>
      <div class="grid cols-2" style="margin-top:16px;">
        <div class="card">
    ...
    if (window.Chart) {
      ...
      const pieCanvas = document.getElementById("advisorPieChart");
      if (pieCanvas && d.perAdvisor.length) {
            ...
            datasets: [
              {
                ...
                backgroundColor: ["#F6C500", "#1B2A6B", "#E23B29", "#1E8E5A", "#C77700", "#565C72", "#101B49"],
              },
            ],
          },
      ...
      const barCanvas = document.getElementById("weekdayBarChart");
      if (barCanvas) {
        barChart = new Chart(barCanvas, {
          type: "bar",
          data: {
            labels: d.chatsByWeekday.map((r) => r.day),
            datasets: [{ label: "Cases attended", data: d.chatsByWeekday.map((r) => r.count), backgroundColor: "#1B2A6B" }],
          },
          ...
          <tbody>${d.advisors
            .map(
              (a) => `<tr>
                ...
                <td><span class="pill ${a.status === "active" ? "green" : a.status === "busy" ? "yellow" : "grey"}" data-advisor-status="${a.id}">${a.status}</span></td>
  ...
  const renderReplies = crudView({
    title: "quick reply",
    listPath: "/api/quick-replies",
    createPath: "/api/admin/quick-replies",
    updatePathFn: (id) => `/api/admin/quick-replies/${id}`,
    deletePathFn: (id) => `/api/admin/quick-replies/${id}`,
    fields: [
      { key: "label", label: "Label (shown as the button text)" },
      { key: "body", label: "Message (use {advisor_name} to insert the advisor's name)", long: true },
    ],
  });
  ...
  async function renderCalendar() {
    ...
    content.innerHTML = `
      <div class="card" style="margin-bottom:16px;">
        ...
        <div class="field"><label>Name</label><input id="calName" placeholder="e.g. Company booking calendar" /></div>
  ...
  async function renderTemplates() {
    content.innerHTML = `<div class="empty">Loading...</div>`;
    const d = await api("/api/proposal-templates");
    content.innerHTML = `
      <div class="card" style="margin-bottom:16px;">
        <h4 style="margin-bottom:10px;">Upload a new background template</h4>
        <p style="color:var(--ink-soft);font-size:13px;margin-top:0;">This image is used as the background only - proposal text is layered on top automatically when an advisor generates a proposal.</p>
        <div class="field"><label>Template name</label><input id="tplName" placeholder="e.g. Leadership Programs" /></div>
        <div class="field"><label>Background image</label><input type="file" id="tplFile" accept="image/*" /></div>
        <button class="btn yellow" id="tplUploadBtn">Add template</button>
      </div>
      <div class="grid cols-3">
        ${d.templates
          .map(
            (t) => `<div class="card">
              <img src="${t.image_url}" style="width:100%;height:120px;object-fit:cover;border-radius:6px;margin-bottom:8px;" />
              <strong>${escapeHtml(t.name)}</strong>
              ${t.is_active ? `<div class="pill green" style="margin-top:6px;">Active template</div>` : ""}
              <div style="display:flex;gap:8px;margin-top:10px;">
                ${t.is_active ? "" : `<button class="btn small ghost" data-select="${t.id}" style="flex:1;">Set as active</button>`}
                <button class="btn small danger" data-delete-tpl="${t.id}" style="flex:1;">Delete</button>
              </div>
            </div>`
          )
          .join("") || `<div class="card empty">No templates uploaded yet.</div>`}
      </div>`;
    ...
    document.getElementById("tplUploadBtn").addEventListener("click", async () => {
      ...
      if (!name || !file) return toast("Give the template a name and choose an image");
      const fd = new FormData();
    ...
    content.querySelectorAll("[data-select]").forEach((b) =>
      b.addEventListener("click", async () => {
        await api(`/api/admin/proposal-templates/${b.dataset.select}/select`, { method: "PUT" });
        toast("Active template updated");
        renderTemplates();
      })
    );
    content.querySelectorAll("[data-delete-tpl]").forEach((b) =>
      b.addEventListener("click", async () => {
        if (!confirm("Delete this template?")) return;
        await api(`/api/admin/proposal-templates/${b.dataset.deleteTpl}`, { method: "DELETE" });
        renderTemplates();
      })
    );
  }
  ...
  async function renderProposals() {
    ...
    content.innerHTML = `<div class="card" style="padding:0;overflow:hidden;">
      <table>
        <thead><tr><th>Company</th><th>Topic</th><th>Staff</th><th>Preferred date</th><th>Status</th><th>Document</th></tr></thead>
        <tbody>${d.proposals
          .map(
            (p) => `<tr>
          ...
          .join("") || `<tr><td colspan="6" class="empty">No proposal requests yet</td></tr>`}
        </tbody>
      </table></div>`;
