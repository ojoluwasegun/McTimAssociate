...
// --- Document uploads (proposal PDFs/Word docs) ---
...
const uploadDoc = multer({
  ...
  fileFilter: (req, file, cb) => {
    const ok = /^(application\/pdf|application\/msword|application\/vnd\.openxmlformats-officedocument\.wordprocessingml\.document|image\/)/.test(
      file.mimetype
    );
    if (!ok) return cb(new Error("Only PDF, Word, or image files are allowed"));
    cb(null, true);
  },
});

router.post("/uploads/document", requireAuth("advisor", "admin"), uploadDoc.single("document"), (req, res) => {
  if (!req.file) return res.status(400).json({ error: "No file uploaded" });
