(function () {
  ...
  async function boot() {
    ...
    applyBadgeLogo(document.getElementById("sidebarBadge"), admin.company_logo);
    showView("analytics");
  }

  function applyBadgeLogo(el, logoUrl) {
    if (!el) return;
    if (logoUrl) {
      el.innerHTML = `<img src="${logoUrl}" alt="Company logo" />`;
    } else {
      el.textContent = "T";
    }
  }
  ...
  function showView(view) {
    ...
    const titles = {
      ...
      templates: "Proposal background templates",
      profile: "Company profile",
    };
    viewTitle.textContent = titles[view] || "";
    const renderers = { analytics: renderAnalytics, advisors: renderAdvisors, visitors: renderVisitors, logs: renderLogs, faqs: renderFaqs, brochures: renderBrochures, quick: renderQuick, proposals: renderProposals, templates: renderTemplates, profile: renderProfile };
    (renderers[view] || (() => {}))();
  }

  // ---------------- ANALYTICS ----------------
  let advisorPieChart = null;
  ...
  async function renderAnalytics() {
    ...
    content.innerHTML = `
      ...
      <div class="grid cols-2" style="margin-top:16px;">
        <div class="card">
          <h4>Case per advisor performance</h4>
          ${d.perAdvisor.length ? `<div style="max-width:320px;margin:0 auto;"><canvas id="advisorPieCanvas"></canvas></div>` : `<div class="empty">No advisors yet</div>`}
        </div>
        <div class="card">
          <h4>Cases attended by day of week</h4>
          <canvas id="weekdayBarCanvas"></canvas>
        </div>
      </div>
      <div class="grid cols-2" style="margin-top:16px;">
        <div class="card">
    ...
    if (advisorPieChart) advisorPieChart.destroy();
    if (weekdayBarChart) weekdayBarChart.destroy();

    if (window.Chart && d.perAdvisor.length) {
      const pieCanvas = document.getElementById("advisorPieCanvas");
      const palette = ["#1B2A6B", "#F6C500", "#E23B29", "#1E8E5A", "#565C72", "#8A6600", "#4A6FE3", "#B5895B"];
      advisorPieChart = new Chart(pieCanvas, {
        type: "pie",
        data: {
          labels: d.perAdvisor.map((a) => a.name),
          datasets: [
            {
              data: d.perAdvisor.map((a) => a.chats_handled || 0),
              backgroundColor: d.perAdvisor.map((_, i) => palette[i % palette.length]),
            },
          ],
        },
    ...
    if (window.Chart) {
      const barCanvas = document.getElementById("weekdayBarCanvas");
      weekdayBarChart = new Chart(barCanvas, {
        type: "bar",
        data: {
          labels: d.chatsByWeekday.map((r) => r.day.slice(0, 3)),
          datasets: [{ label: "Cases attended", data: d.chatsByWeekday.map((r) => r.c), backgroundColor: "#1B2A6B" }],
        },
        options: {
  ...
  async function renderProposals() {
    content.innerHTML = `<div class="empty">Loading...</div>`;
    const [d, t] = await Promise.all([api("/api/admin/proposals"), api("/api/proposal-templates")]);
    const templates = t.templates;
    if (!d.proposals.length) {
      content.innerHTML = `<div class="card empty">No proposal requests yet.</div>`;
      return;
    }
    content.innerHTML = `<div class="grid cols-2">${d.proposals.map((p) => proposalCardHtml(p, templates)).join("")}</div>`;
    wireProposalCard(content, templates, renderProposals);
  }

  function proposalCardHtml(p, templates) {
    const tpl = templates.find((x) => x.id === p.template_id);
    const bg = tpl ? `background-image:url('${tpl.image_url}');` : "";
    return `
      <div class="proposal-card" style="${bg}">
        <div class="proposal-card-inner">
          <strong>${escapeHtml(p.company_name || "-")}</strong>
          <div style="font-size:13px;color:var(--ink-soft);margin:4px 0 10px;">${escapeHtml(p.training_topic || "-")}</div>
          <div style="font-size:12.5px;color:var(--ink-soft);margin-bottom:10px;">Staff: ${escapeHtml(p.staff_count || "-")} &middot; Preferred date: ${escapeHtml(p.preferred_date || "-")}</div>
          <div class="field"><label>Status</label>
            <select data-status="${p.id}">${["new", "in_progress", "sent", "closed"].map((s) => `<option value="${s}" ${s === p.status ? "selected" : ""}>${s}</option>`).join("")}</select>
          </div>
          <div class="field"><label>Background template</label>
            <select data-template="${p.id}">
              <option value="">None</option>
              ${templates.map((tp) => `<option value="${tp.id}" ${tp.id === p.template_id ? "selected" : ""}>${escapeHtml(tp.name)}</option>`).join("")}
            </select>
          </div>
          <div class="field">
            <label>Proposal document</label>
            ${p.document_url ? `<a href="${p.document_url}" target="_blank" style="font-size:13px;">View uploaded document</a><br/>` : ""}
            <input type="file" data-doc="${p.id}" accept=".pdf,.doc,.docx,image/*" />
          </div>
        </div>
      </div>`;
  }

  function wireProposalCard(root, templates, rerender) {
    ...
    root.querySelectorAll("[data-template]").forEach((sel) =>
      sel.addEventListener("change", async () => {
        const template_id = sel.value ? Number(sel.value) : null;
        await api(`/api/admin/proposals/${sel.dataset.template}`, { method: "PUT", body: JSON.stringify({ template_id }) });
        toast("Background updated");
        rerender();
      })
    );
    root.querySelectorAll("[data-doc]").forEach((input) =>
      input.addEventListener("change", async (e) => {
        ...
        if (out.url) {
          await api(`/api/admin/proposals/${input.dataset.doc}`, { method: "PUT", body: JSON.stringify({ document_url: out.url }) });
          toast("Document uploaded");
          rerender();
        } else {
          toast(out.error || "Upload failed");
        }
      })
    );
  }
  ...
  async function renderTemplates() {
    content.innerHTML = `<div class="empty">Loading...</div>`;
    const t = await api("/api/proposal-templates");
    content.innerHTML = `
      <div class="card" style="margin-bottom:16px;max-width:420px;">
        <h4 style="margin-top:0;">Upload a background template</h4>
        <div class="field"><label>Template name</label><input id="tplName" placeholder="e.g. Corporate Blue" /></div>
        <div class="field"><label>Background image</label><input type="file" id="tplFile" accept="image/*" /></div>
        <button class="btn yellow" id="tplUploadBtn">Upload template</button>
      </div>
      <div class="grid cols-3">${t.templates
        .map(
          (tp) => `<div class="card" style="padding:0;overflow:hidden;">
            <div style="height:110px;background-image:url('${tp.image_url}');background-size:cover;background-position:center;"></div>
            <div style="padding:10px;display:flex;justify-content:space-between;align-items:center;">
              <span style="font-size:13px;">${escapeHtml(tp.name)}</span>
              <button class="btn small danger" data-del-tpl="${tp.id}">Delete</button>
            </div>
          </div>`
        )
        .join("") || `<div class="card empty">No background templates uploaded yet.</div>`}
      </div>`;

    document.getElementById("tplUploadBtn").addEventListener("click", async () => {
      const name = document.getElementById("tplName").value.trim();
      const file = document.getElementById("tplFile").files[0];
      if (!name || !file) return toast("Give the template a name and pick an image");
      const fd = new FormData();
      fd.append("image", file);
      const res = await fetch("/api/uploads/image", { method: "POST", headers: { Authorization: `Bearer ${token}` }, body: fd });
      const out = await res.json();
      if (!out.url) return toast(out.error || "Upload failed");
      await api("/api/admin/proposal-templates", { method: "POST", body: JSON.stringify({ name, image_url: out.url }) });
    ...
    content.querySelectorAll("[data-del-tpl]").forEach((btn) =>
      btn.addEventListener("click", async () => {
        ...
        await api(`/api/admin/proposal-templates/${btn.dataset.delTpl}`, { method: "DELETE" });
        renderTemplates();
      })
    );
  }
