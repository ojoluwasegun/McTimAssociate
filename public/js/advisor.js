(function () {
  ...
  async function applyBranding() {
    try {
      ...
      if (b.company_logo) {
        if (badge) badge.innerHTML = `<img src="${b.company_logo}" alt="Company logo" />`;
        if (ringBadge) ringBadge.innerHTML = `<img src="${b.company_logo}" alt="Company logo" style="width:100%;height:100%;object-fit:cover;border-radius:50%;" />`;
      }
      const brandNameEl = document.querySelector(".sidebar .brand-name strong");
      if (brandNameEl && b.company_name) brandNameEl.textContent = b.company_name;
    } catch (e) {
  ...
  function connectSocket() {
    ...
    socket.on("chat_claimed", ({ chatId }) => {
      ...
      if (currentChatId && m.chat_id === currentChatId && document.body.dataset.view === "dashboard") {
        // Advisor's own sends are already rendered optimistically in sendDashMessage - skip the echo
        if (m.sender_type === "advisor") return;
        appendDashboardMessage(m);
      }
    });
    socket.on("client_message", ({ chatId, body }) => {
      // "new_message" (joined via chat room) already renders this when we're on the dashboard viewing
      // this exact chat - this event is just a lightweight nudge for other tabs/views.
  ...
  async function renderDashboard() {
    ...
    document.getElementById("quickRepliesWrap").appendChild(renderQuickReplies());
  ...
  async function sendDashMessage(presetText) {
    const input = document.getElementById("dashInput");
    const body = (presetText !== undefined ? presetText : input.value).trim();
    if (!body || !currentChatId) return;
    if (presetText === undefined) input.value = "";
    // Render immediately on the advisor's own screen (don't wait on the socket round-trip)
    appendDashboardMessage({ sender_type: "advisor", sender_name: advisor.name, body });
    try {
  ...
  const QUICK_REPLIES = [
    "Hi, thanks so much for waiting - I'm here now, how can I help?",
    "Just following up - is there anything else you need from me?",
    "Thanks for reaching out to McTimothy Associates! Let me look into that for you.",
  ];
  ...
  function renderQuickReplies() {
    ...
    QUICK_REPLIES.forEach((text) => {
      ...
      btn.title = text;
      btn.addEventListener("click", () => sendDashMessage(text));
  ...
  async function renderProposals() {
    content.innerHTML = `<div class="empty">Loading...</div>`;
    const [data, t] = await Promise.all([api("/api/admin/proposals"), api("/api/proposal-templates")]);
    const templates = t.templates;
    if (!data.proposals.length) {
      content.innerHTML = `<div class="card empty">No proposal requests yet.</div>`;
      return;
    }
    content.innerHTML = `<div class="grid cols-2">${data.proposals.map((p) => proposalCardHtml(p, templates)).join("")}</div>`;
