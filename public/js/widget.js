  ...
  (async function applyBranding() {
    try {
      ...
      if (b.company_logo) {
        ...
        if (avatarEl) avatarEl.innerHTML = `<img src="${b.company_logo}" alt="Company logo" />`;
      }
      if (b.company_name) {
        const titleEl = document.getElementById("widgetTitle");
        if (titleEl) titleEl.textContent = `Tim - ${b.company_name}`;
      }
    } catch (e) {
  ...
  function connectSocket() {
    ...
    socket.on("chat_status", (s) => {
      ...
      if (s.status === "ringing") {
        ...
        // Free up vertical space for the conversation now that a human has joined
        const qa = document.getElementById("quickActions");
        if (qa) qa.classList.add("collapsed");
